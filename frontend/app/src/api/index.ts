...
export const strapiBase = `http://localhost:${import.meta.env.VITE_STRAPI_PORT}`;
...
export function getListFactory(contentType: string, populate: string[] = [], imageSize: ImageSize = 'original', sort: string = 'publishedAt:desc') {
  ...
  function mapItem({id, attributes: {title, slug, summary, publishedAt, image}}) {
    return {
      id, title, slug, summary, publishedAt, image: populate.length ? image.data.map(mapImage):[],
    };
  }
  ...
  async function getList() {
    ...
    return axios.get(uri, config).then(({data: {data}}) => data.map(mapItem));
  }
...
export function getItemFactory(slug: string|undefined, contentType: string, populate: string[] = [], imageSize: ImageSize = 'original') {
  function mapImage({id, attributes: {alternativeText, caption, formats, url}}) {
    const imageUrl = imageSize==='original' ?
      url:
      formats[imageSize].url;
  ...
  function mapItem({id, attributes: {title, slug, summary, text, publishedAt, image}}) {
    return {
  ...
  async function getItem() {
    ...
    const uri = `${strapiBase}/api/${contentType}`;
    return axios.get(uri, config).then(({data: {data}}) => mapItem(data[0]));
  }
